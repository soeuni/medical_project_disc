...
from dataset_test import get_df_stone, get_transforms, MMC_ClassificationDataset
...
def confidence_score(PROBS, TARGETS, IMAGE_NAME, threshold_list):
    ...
    confidence_dict = {'image_name': IMAGE_NAME[0],
                       'prob1': PROBS[:, 0],
                       'prob2': PROBS[:, 1],
                       'prob3': PROBS[:, 2],
                       'prob4': PROBS[:, 3],
...
def sorted_confidence_save(select):
    ...
    for img_name in high_confidence:
        shutil.copyfile(f'./data/images/test/{img_name}', f'./confidence/{args.kernel_type}_high_confidence_image/{img_name}')

def plot_roc_curve(fper, tper, optimal_threshold, level):
    # plt.figure()
...
def val_epoch(model, loader, fold, n_test=1):
    '''
    ...
    #     # roc plot
    #     plot_roc_curve(fpr, tpr, fold, ???(optimal_threshold값), level)
    #
    ...
    # return LOGITS, PROBS, TARGETS, PATIENT_ID, IMAGE_NAME, acc, auc_list, threshold_list
    return PROBS, TARGETS, IMAGE_NAME
...
def main():
    ...
    ####################################################
    '''
    df_test, meta_features, n_meta_features = get_df_stone(
        k_fold=args.k_fold,
    ...
    transforms_test = get_transforms(args.image_size)
    ...
    sum_PROBS = torch.zeros((df_test.shape[0], args.out_dim))
    ...
    for fold in folds:
        ...
        if len(df_test) % args.batch_size == 1:
            df_test = df_test.sample(len(df_test) - 1)

        # if args.DEBUG:
        #     df_test = df_test.sample(args.batch_size * 100)

        dataset_test = MMC_ClassificationDataset(df_test, 'valid', meta_features, transform=transforms_test)
        ...
        ####################################################
        '''
        # this_LOGITS, this_PROBS, this_TARGETS, this_PATIENT_ID, this_IMAGE_NAME, acc, auc_list, threshold_list = val_epoch(model, test_loader, fold, n_test=8)
        this_PROBS, this_TARGETS, this_IMAGE_NAME = val_epoch(model, test_loader, fold, n_test=8)
        # LOGITS.append(this_LOGITS)
        sum_PROBS += this_PROBS
    ...
    mean_PROBS = sum_PROBS / len(folds)
    # AUC : ROC 아래 넓이
    auc_list = np.zeros(args.out_dim)
    ...
    # threshold_list = []
    for level in range(args.out_dim):
        auc_list[level] = roc_auc_score((TARGETS[:, level]).astype(float), mean_PROBS[:, level].numpy())
    #
    #     plt.subplot(2,3,level+1)
    #     # roc-curve
    #     fpr, tpr, threshold = roc_curve((TARGETS[:, level]).astype(float), PROBS[:, level])
    ...
    # plt.savefig(f'./roc_curve_image/{args.kernel_type}_roc_curve_image/{args.kernel_type}_fold{fold}.png')
    ##################################################################################################
    threshold_list = [0.0417, 0.0717, 0.1236, 0.2569, 0.209]  # 4개의 fold의 각 level별 threshold 평균

    zero = (mean_PROBS[:,0] > threshold_list[0]).reshape(-1, 1)
    one = (mean_PROBS[:, 1] > threshold_list[1]).reshape(-1, 1)
    two = (mean_PROBS[:, 2] > threshold_list[2]).reshape(-1, 1)
    three = (mean_PROBS[:, 3] > threshold_list[3]).reshape(-1, 1)
    four = (mean_PROBS[:, 4] > threshold_list[4]).reshape(-1, 1)
    after_PROBS = np.concatenate([zero, one, two, three, four], axis=1)
    ...
    confidence_score(mean_PROBS, TARGETS, IMAGE_NAME, threshold_list)
...
if __name__ == '__main__':
    ...
    os.makedirs('./test_confidence', exist_ok=True)
    os.makedirs(f'./test_confidence/{args.kernel_type}_high_confidence_image', exist_ok=True)
    os.makedirs(f'./test_roc_curve_image/{args.kernel_type}_roc_curve_image', exist_ok=True)
